...
export async function commitBranch(options: Options) {
  ...
  const git2: SimpleGit = simpleGit().outputHandler(
    (_command, stdout, stderr) => {
  ...
  //   // stdio: "inherit", // This will inherit the parent process's stdio
  //   trimmed: false,
  // });

  // try {
  await git2.commit(title, noVerify ? ["--no-verify"] : undefined);
  // } catch (error) {
